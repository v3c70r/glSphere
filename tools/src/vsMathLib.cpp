...
void VSMathLib::loadIdentity(ComputedMatrixTypes aType)
{
    if (aType == NORMAL)
        setIdentityMatrix(mCompMatrix[aType]);
}
...
void
VSMathLib::createRotationMatrix(float angle, float x, float y, float z, float * res)
{
    

	float radAngle = angle;//DegToRad(angle);
...
VSMathLib::rotate(MatrixTypes aType, float angle, float x, float y, float z)
{
    ...
    createRotationMatrix(angle, x, y, z, mat);
...
void 
VSMathLib::get(MatrixTypes aType, float* res)
{
    memcpy(res, mMatrix[aType], sizeof(float)*16);
}


// returns a pointer to the requested matrix
void
VSMathLib::get(ComputedMatrixTypes aType, float* res)
{
	switch (aType) {
	
		case NORMAL: 
			computeNormalMatrix3x3();
            memcpy(res, mNormal3x3, sizeof(float)*9);
			break;
		default:
...
//res = map a point on screen to sphere
void VSMathLib::mapScreen2Sphere( float x, float y, float w, float h, float *res){
